...
//农夫NPC
public class IvyNPC extends NPC {

    public IvyNPC(Long id, String name) {
        super(id, name);
    }
    ...
    protected void registAction() {
        ...
        this.addAction(new MakeCakeAction(null));
    }

    @Override
    protected CharacterType npcType() {
        return CharacterType.Ivy;
    }
        ...
        if (todo.isEmpty() && current == 0) {
            ...
            if (oid.equals("moveTo")) {
               ...
               // doAction(ActionEnumType.Move.getCode(), params);
                ...
                params.put("oid", oid);
                int actionId = 0;
                if (oid.equals("ivyCake")) {
                    actionId = ActionEnumType.MakeCake.getCode();
                }  else {
