...
//农夫NPC
public class AidenNPC extends NPC {

    public AidenNPC(Long id, String name) {
        super(id, name);
    }
    ...
    protected void registAction() {
        ...
        this.addAction(new SportAction(null));
    }

    @Override
    protected CharacterType npcType() {
        return CharacterType.Aiden;
    }
        ...
        if (todo.isEmpty() && current == 0) {
            ...
            if (oid.equals("moveTo")) {
               ...
               // doAction(ActionEnumType.Move.getCode(), params);
                ...
                params.put("oid", oid);
                int actionId = 0;
                if (oid.equals("aidenSport")) {
                    actionId = ActionEnumType.Sport.getCode();
                }  else {
