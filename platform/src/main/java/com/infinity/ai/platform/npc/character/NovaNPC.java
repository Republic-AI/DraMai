...
//农夫NPC
public class NovaNPC extends NPC {

    public NovaNPC(Long id, String name) {
        super(id, name);
    }
    ...
    protected void registAction() {
        ...
        this.addAction(new WaterAction(null));
    }

    @Override
    protected CharacterType npcType() {
        return CharacterType.Nova;
    }
        ...
        if (todo.isEmpty() && current == 0) {
            ...
            if (oid.equals("moveTo")) {
               ...
               // doAction(ActionEnumType.Move.getCode(), params);
                ...
                params.put("oid", oid);
                int actionId = 0;
                if (oid.equals("novaWater")) {
                    actionId = ActionEnumType.Water.getCode();
                }  else {
