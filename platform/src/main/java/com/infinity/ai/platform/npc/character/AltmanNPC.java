...
//农夫NPC
public class AltmanNPC extends NPC {

    public AltmanNPC(Long id, String name) {
        super(id, name);
    }
    ...
    protected void init() {
        ...
        this.oidList.add("altmanSleep");
        this.oidList.add("altmanEat");
        this.oidList.add("altmanCook");
        this.oidList.add("altmanToAI");
    }
    ...
    protected void registAction() {
        ...
        this.addAction(new ToAiAction(null));
    }

    @Override
    protected CharacterType npcType() {
        return CharacterType.Altman;
    }
        ...
        if (todo.isEmpty() && current == 0) {
            ...
            if (oid.equals("moveTo")) {
               ...
               // doAction(ActionEnumType.Move.getCode(), params);
                ...
                params.put("oid", oid);
                int actionId = 0;
                if (oid.equals("altmanSleep")) {
                    actionId = ActionEnumType.Sleep.getCode();
                }  else if (oid.equals("altmanEat")) {
                    actionId = ActionEnumType.Dinning.getCode();
                } else if (oid.equals("altmanCook")) {
                    actionId = ActionEnumType.Cook.getCode();
                } else {
                    actionId = ActionEnumType.ToAi.getCode();
                }
